...
const insertPersonnel = (
  ...
  const query =
    "INSERT INTO `Personnels`( `email`, `password`, `nomPersonnel`, `prenomPersonnel`, `telephone`, `adresse`, `dateEmbauche`, `grade`, `pseudo`, `salaire`, `position`) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
  con.query(
    query,
    [
      email,
...
router.put("/", (req, res) => {
  const query =
    "UPDATE `Personnels` SET `nomPersonnel`=?,`prenomPersonnel`=?,`telephone`=?,`adresse`=?,`pseudo`=?,`salaire`=?,`position`=? WHERE cin=?";
  const {
    nomPersonnel,
    prenomPersonnel,
    position,
